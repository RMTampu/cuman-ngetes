...
object LocalSocksServer {
    ...
    fun start() {
        ...
        ProbeDiagnostics.setSocksListening(true)
        pool!!.execute {
            while (running.get()) {
                try {
                    val socket = server?.accept() ?: break
                    ProbeDiagnostics.socksAccepted(socket.remoteSocketAddress?.toString())
    ...
    fun stop() {
        ...
        pool?.shutdownNow()
        pool = null
        ProbeDiagnostics.setSocksListening(false)
    }
    ...
    private fun handleConnect(clientIn: InputStream, clientOut: OutputStream, host: String, port: Int) {
        val remote = try {
            ...
            ProbeDiagnostics.tcpConnect(host, port, false, t.message)
            reply(clientOut, 0x05, null, 0)
            return
        }

        ProbeDiagnostics.tcpConnect(host, port, true)
        remote.use { r ->
            reply(clientOut, 0x00, r.localAddress, r.localPort)
            ProbeBus.emit(ProbeEvent(host = host, port = port, direction = Direction.CONNECT, sizeBytes = 0))
    ...
    private fun relay(input: InputStream, output: OutputStream, host: String, port: Int, direction: Direction) {
        ...
        try {
            while (running.get()) {
                ...
                output.flush()
                ProbeDiagnostics.tcpBytes(direction == Direction.OUT, host, port, n)
                ProbeBus.emit(ProbeEvent(host = host, port = port, direction = direction, sizeBytes = n))
            }
        } catch (_: Throwable) { }
    }
            ...
            try {
                while (running.get() && !udp.isClosed) {
                    ...
                    if (clientEndpoint == null || sender == clientEndpoint) {
                        ...
                        ProbeDiagnostics.udpPacket(true, host, port, payloadLen)
                        ProbeBus.emit(ProbeEvent(host = host, port = port, direction = Direction.UDP_OUT, sizeBytes = payloadLen))
                    } else {
                        ...
                        ProbeDiagnostics.udpPacket(false, hp.first, hp.second, packet.length)
                        ProbeBus.emit(ProbeEvent(host = hp.first, port = hp.second, direction = Direction.UDP_IN, sizeBytes = packet.length))
                    }
                }
            } catch (_: Throwable) { }
        }
